...
a_0, a_1, p_d = symbols("a_0 a_1 p_d",   real = True)
# def func(x, a_0, a_1):
#     return a_0*(x)/(np.log(x/a_1) + 1)

expr = 'a_0*(p_d)/(log(p_d/a_1) + 1)'
...
with plt.style.context('seaborn-whitegrid'):
    ...
    lam_x = lambdify([p_d, a_0, a_1], expr, modules=['numpy'])
    l, = plt.plot(pds_auxiliar, lam_x(pds_auxiliar, A_0, A_1), linewidth = 2)
    ...
    s_1 = Slider(ax_1, label = 'a_1', valmin = .5, valmax = .75, valinit = A_1, valstep = .001)
